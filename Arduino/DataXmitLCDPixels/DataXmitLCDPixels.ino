...
#define PACKSIZE 25 // Maximum Packetsize is 23,if 4 digit for x and y, and static characters
...
int gxcoor, gycoor, bxcoor, bycoor,yxcoor, yycoor;
...
{ 
  ...
  lcd.setCursor(0,0);
    for (i=0; i<1000; i++) {
      ...
      // Construct a packet called data. Show sample number and green ball x, y coords.
      sprintf(data, "TX#:%d G<%d,%d>", i, gxcoor, gycoor);
      sendlength = sizeof(data); // measure the constructed packet
      lcd.setCursor(2,0); //Green ball coords on 1st line, after activity char
